...
\begin{abstract}
    ...
    To accurately model these agents and other complex phenomena, machine learning agents need to get better at replicating these capabilities.
    When only provided with partial information about a system, current agents rely on sufficiently complex internal structures to enable them to learn complex tasks, yet the question remains how these internal structures come to be.
    An information theoretical approach to internal model generation was explored with a simple delayed action task.
...
\begin{figure}[H]
    ...
    \caption{\label{fig:obs_L_mdp} The process of reducing the delayed action task (Fig. \ref{fig:delayed_mdp}) to the observable light states. One can see that $S_1$ and $S_2$ produce the same observation.}
\end{figure}
...
The results presented show that information theoretical measures can be applied to a Partially Observable Markov Process in order to find accurate internal models, though mostly analytically.
From observing the light state of the system, one can obtain an internal model which would enable an agent to learn the delayed action task.
...
Creating more realistic and capable agents promises a more complete understanding of agency and the emergence of complex behaviour in complex adaptive systems.
